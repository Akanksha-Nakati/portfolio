...
const typeStyles: Record<string, { dot: string; tag: string }> = {
  origin: { dot: "bg-[#A89EF0]", tag: "bg-[#A89EF0]/20 text-[#4838C9]" },
  growth: { dot: "bg-[#6B5CE7]", tag: "bg-[#6B5CE7]/10 text-[#4838C9]" },
  pivot:  { dot: "bg-[#FFD4B8]", tag: "bg-[#FFD4B8]/40 text-[#A0500A]" },
  now:    { dot: "bg-[#6B5CE7] ring-4 ring-[#6B5CE7]/20", tag: "bg-[#6B5CE7] text-white" },
};
  ...
  return (
    <>
      <main className="bg-[#F0ECFF] pt-28 pb-0">
        ...
        <div className="px-8 md:px-14 py-16 border-b border-[#C8C0E8]">
          <div className="w-full max-w-[1400px] mx-auto flex flex-col md:flex-row items-start md:items-end justify-between gap-10">
            <div className="flex flex-col gap-4">
              <span className="text-[10px] font-semibold uppercase tracking-[0.25em] text-[#6B5CE7]">About</span>
              <h1 className="font-[var(--font-display)] text-[clamp(56px,9vw,120px)] leading-[0.88] tracking-tight text-[#1A1040]">
                About<br />
                <span className="italic text-[#6B5CE7]">Me.</span>
              </h1>
            </div>
          </div>
        </div>
        ...
        {/* Bio */}
        <div className="px-8 md:px-14 py-20 border-b border-[#C8C0E8]">
          <div className="w-full max-w-[1400px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-12 items-start">
            {/* Text, 2 cols */}
            <div className="md:col-span-2 flex flex-col gap-10">
              <p className="font-[var(--font-display)] italic text-[clamp(22px,2.8vw,34px)] leading-[1.3] text-[#1A1040]/80 max-w-[700px]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                <p className="text-[15px] leading-relaxed text-[#7B72A8]">
            ...
            <div className="flex items-start justify-center md:justify-end pt-4">
              <div className="relative w-[180px] h-[180px] md:w-[210px] md:h-[210px] rounded-full ring-4 ring-[#6B5CE7] bg-[#F0ECFF] overflow-hidden">
                <Image
        ...
        <div className="px-8 md:px-14 py-20 pb-32">
          <div className="w-full max-w-[1400px] mx-auto">
            <div className="flex flex-col gap-3 mb-16">
              <span className="text-[10px] font-semibold uppercase tracking-[0.25em] text-[#6B5CE7]">The Journey</span>
              <h2 className="font-[var(--font-display)] text-[clamp(32px,5vw,64px)] leading-none text-[#1A1040] tracking-tight">
                Mumbai → Boston ✈️<br />
                <span className="italic text-[#6B5CE7]">one dataset at a time.</span>
              </h2>
            </div>
                ...
                return (
                  <div key={i} className="relative flex gap-8 md:gap-14">
                    ...
                    <div className="hidden md:flex flex-col items-end w-[90px] shrink-0 pt-1">
                      <span className="font-[var(--font-display)] italic text-[#6B5CE7] text-sm leading-none text-right">
                        {item.year}
                      </span>
                    </div>
                    ...
                    <div className="flex flex-col items-center shrink-0">
                      ...
                      {!isLast && <div className="w-px flex-1 bg-[#C8C0E8] mt-2 min-h-[40px]" />}
                    </div>

                    {/* Content */}
                    <div className="flex flex-col gap-4 pb-14 flex-1">
                      <span className="md:hidden font-[var(--font-display)] italic text-[#6B5CE7] text-sm">
                        {item.year}
                      </span>
                      <div className="flex items-center gap-3 flex-wrap">
                        <span className="font-[var(--font-display)] text-[clamp(20px,2.5vw,30px)] text-[#1A1040] leading-none">
                          {item.company}
                        </span>
                        {item.type === "now" && (
                          <span className="text-[9px] font-bold uppercase tracking-widest px-2.5 py-1 bg-[#6B5CE7] text-white rounded-full animate-pulse">
                            Current
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-sm text-[#7B72A8]">{item.company}</span>
                        <span className="text-[#C8C0E8]">·</span>
                        <span className="text-sm font-bold text-[#1A1040]">{item.role}</span>
                        <span className="text-[#C8C0E8]">·</span>
                        <span className="text-xs text-[#7B72A8] uppercase tracking-widest">{item.location}</span>
                      </div>
                      <p className="text-[15px] leading-[1.75] text-[#7B72A8] max-w-[640px]">
                        {item.story}
                      </p>
                      {item.highlights.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                          {item.highlights.map((h) => (
                            <span key={h} className="text-xs px-3 py-1.5 rounded-lg bg-[#E8E2FF] border border-[#C8C0E8] text-[#1A1040]/70">
                              {h}
                            </span>
                          ))}
                        </div>
